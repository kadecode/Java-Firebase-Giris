...
public class Kelimelerdao {
    ...
    public int kayitSorgu(VeritabaniYardimcisi vt) {
        Integer sonuc = 0;
        SQLiteDatabase dbx = vt.getWritableDatabase();
    ...
    public Kelimeler kelimeGetir(VeritabaniYardimcisi vt, int kelime_id) {
        ...
        Cursor c = dbx.rawQuery("SELECT * FROM kelimler WHERE kelime_id=?" + kelime_id, null);
    ...
    public ArrayList<Kelimeler> tumKelimelerRasgele5(VeritabaniYardimcisi vt) {
        ArrayList<Kelimeler> kelimelerArrayList = new ArrayList<>();
        SQLiteDatabase dbx = vt.getWritableDatabase();
        Cursor c = dbx.rawQuery("SELECT * FROM kelimeler ORDER BY RANDOM() LIMIT 5", null);
        while (c.moveToNext()) {
            Kelimeler kelime = new Kelimeler(c.getInt(c.getColumnIndex("kelime_id"))
                    , c.getString(c.getColumnIndex("ingilizce"))
                    , c.getString(c.getColumnIndex("turkce")));
